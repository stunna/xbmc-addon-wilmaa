...
def get_prgtitle(station, prgtitle, prgtitlespielfilm=''):
    ...
    if i == 0:
        ...
        while 0 < i2:
            if prgtitlespielfilm[i2]['title'].find(station) > 0:
                return str(prgtitlespielfilm[i2]['title'])
            i2 -= 1
...
add_video_item('%s/%s.m3u8' % (base_url, 'sat1'),{ 'title': 'SAT1' + ' - ' + get_prgtitle('SAT.1', prgtitle, prgtitle_spielfilm) }, '%s/media/sat1.png' % _resdir)
...
add_video_item('%s/%s.m3u8' % (base_url, 'pro7'),{ 'title': 'PRO7' + ' - ' + get_prgtitle('ProSieben', prgtitle, prgtitle_spielfilm) }, '%s/media/pro7.png' % _resdir)
...
add_video_item('%s/%s.m3u8' % (base_url, 'rtlnitro'),{ 'title': 'RTLNitro' + ' - ' + get_prgtitle('RTL NITRO:', prgtitle, prgtitle_spielfilm) }, '%s/media/rtlnitro.png' % _resdir)
